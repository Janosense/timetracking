...
`buildCompetitionResponse` (same file) is the single source of truth for the `/api/competitions/[id]` response. It joins competitions, participants, and finish records, computes derived fields (`currentLap`, per-participant `laps[]`, `completedLaps`, `lastLapTimeMs`, `totalTimeMs`, `currentLapFinished`), and is returned by both the GET endpoint and the finish POST. Client pages key off this shape — extend it rather than creating parallel endpoints.
...
- Composables: `useAdminAuth` (login/logout/checkAuth), `useFormatTime` (ms → display), `useCsvParser` (participant import).
